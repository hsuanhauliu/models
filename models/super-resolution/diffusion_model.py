...
class SinusoidalPositionEmbeddings(nn.Module):
    ...
    Explanation: https://learnopencv.com/sinusoidal-position-embeddings/
    """
...
if __name__ == '__main__':
    ...
    low_res_images_upscaled = torch.randn(batch_size, 3, img_size, img_size)
    timesteps = torch.randint(0, 1000, (batch_size,)).long()

    # The UNet takes the noisy image and the upscaled low-res image as input
    predicted_noise = model(noisy_images, timesteps, low_res_images_upscaled)
    
    print(f"\nInput noisy image shape: {noisy_images.shape}")
    print(f"Input low-res shape: {low_res_images_upscaled.shape}")
